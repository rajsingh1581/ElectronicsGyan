...
// Robust generation function with exponential backoff retry and model fallback
async function generateWithRetryAndFallback(
  ...
  for (const model of modelsToTry) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        console.log(`Attempting generation with model ${model} (attempt ${attempt + 1}/${maxRetries})...`);
        ...
        if (isTransient && attempt < maxRetries - 1) {
          console.warn(`Transient error on ${model} (attempt ${attempt + 1}). Retrying in ${delay}ms...`, error);
          await new Promise((resolve) => setTimeout(resolve, delay));
          delay *= 2;
        } else {
          // Break to fallback to next model if this was not a retryable error or we exhausted retries
          break;
        }
      }
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const systemInstruction = "You are Kiara, the highly knowledgeable, brilliant, and polite female AI Assistant and Engineering Expert for Electronics Gyan, an elite engineering and technology platform. " +
    ...
    const response = await generateWithRetryAndFallback(ai, contents, systemInstruction);
